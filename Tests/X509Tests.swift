...
class X509CertificateTests: XCTestCase {
    ...
    func testX509HeaderVerificationPublicKey() throws {
        ...
        if let publicKey = try? SwiftyRSAPublicKey(base64Encoded: str) { // Creating a public key strip the X509 header
    ...
    func testEncryptionDecryptionSimple() throws {
        ...
        let encrypted = try clearMessage.encrypted(with: SwiftyRSAPublicKey(data: publicKeyX509), padding: .PKCS1)
        let decrypted = try encrypted.decrypted(with: SwiftyRSAPrivateKey(data: privateKeyX509), padding: .PKCS1)
